...
- The UI follows the reference: global Desired rates, All production grand total, nested group
  ...
  group Ignore passes both sides to the parent. A compact status strip sits between bounded Desired rates and right-aligned power. Its shaded Whole setup panel covers every group. It shows live success/failure, actual recipe activity and boundary counts; the All production material-rule counts are root-only. Material rules opens two short definitions, a compact water example explicitly labeled Example, and one sentence about parent sharing without expanding the layout. Keep this guide brief; no count tables or long explanations. Match/Ignore hover text is only a short phrase for the selected rule. Yellow chance-percentage overlays on item icons are removed globally; chance information remains in hover details and solver calculations. Target warnings focus their explanation. There is no general never-import rule. Do not restore separate Resources/Materials panels or three-way rule selects.
