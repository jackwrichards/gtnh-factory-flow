...
- The UI follows the reference: global Desired rates, All production grand total, nested group
  ...
  group Ignore passes both sides to the parent. Desired rates fills the top row beside right-aligned power, with separate Item / Rule / Target / Actual / Status columns. Failed statuses open the existing target explanation. All production is the shared heading above Status / Inputs / Outputs / Internal, with matching section styling. Rate statuses use machine-style dots and explicit phrases. All four sections share one grid: four across, then two by two, then a single column only on phones. Status must not reserve a separate vertical lane. Compress the rate-status wording and numeric columns before wrapping desired-rate items; phone items use two lines at most, never overlapping headings or horizontal scrolling. Material columns pack to their actual rate text and the small Skip toggle, adding columns whenever they fit; machine counts reserve a consistent width so voltage chips align. Its Status section covers every group. It shows live success/failure, actual recipe activity and boundary counts; the All production material-rule counts are root-only. Rules example opens a short water example explicitly labeled Example, using the same status headings and input → recipes → output component as the real status card, plus one sentence about parent sharing. Keep this guide brief; no count tables or long explanations. It uses useDropdownDismiss with fade, the shared pageOpen/pageClose sounds, and no separate close button. Match/Ignore hover text is only a short phrase for the selected rule. Yellow chance-percentage overlays on item icons are removed globally; chance information remains in hover details and solver calculations. Takes/Makes rows show compact item icons without inline rates; use the existing item tooltip for rates. Keep non-consumed markers visible. Material rules use a small Skip toggle button: off is Match, highlighted is Ignore; hover states the rule. Preserve saved legacy direct-import rules until the user changes them. Target warnings focus their explanation. There is no general never-import rule. Do not restore separate Resources/Materials panels or three-way rule selects.
