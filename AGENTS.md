...
- LINE LABELS ARE GONE (Jack, 2026-09-08: "dropping support for line
  ...
  not the original resource colour. Hovering any percentage opens a compact live
  overview of that drawer's incoming/outgoing shares and Setup output, with a
  wheel up/down hint (Shift: 10%). It uses the shared tooltip and never intercepts
  the pointer. Preview graph subscriptions mount only while open. No special cursor. The drawer
...
- The UI follows the reference: global Desired rates, All production grand total, nested group
  ...
  has its icon, signed net rate, and Balance toggle on one line; columns use
  ...
  with every material visible, no search or pagination; never truncate material rates or add internal scrollbars. When net flow is zero, use recipe ports to keep ingredients under Inputs, products under Outputs, and paired materials under Internal; source/product drawers do not make recipe boundaries internal. Child-local totals without
  a parent material row keep their controls in the child. The underlying rules are Match / Ignore, as in
  Shadow. Match balances paired materials exactly; root Ignore permits imports and surplus,
  group Ignore passes both sides to the parent. Desired rates fills the top row beside right-aligned power, with one header row: Desired rates (Drag items here) / Rule / Target / Actual / Status, with + above the remove buttons. Failed statuses open the existing target explanation. All production heads three full-width material sections: Inputs / Outputs / Internal, wrapping only as needed. There is no setup Status dashboard, repeated rule/activity counts, or Rules example panel. Desired-rate statuses tell whether targets are met; Why opens only the selected failed target in a dismissible popup using useDropdownDismiss with fade and shared open/close sounds. Do not invent a blocking cause when the solver has not identified one. Missing-recipe calculation errors remain visible as a compact message. Rate statuses use machine-style dots and explicit phrases, shortening before desired-rate rows wrap; phone items use two lines at most. Desired-rate Target and Actual text use worksheet-sized type. Materials can be picked up by their icon, rate, or surrounding row space; Takes/Makes icons have a forgiving invisible pickup margin. A machine row can be dragged with the mouse from its name, status, power, blank space, or controls, using the same reorder/group-drop behavior as its grip. Takes/Makes items retain their own resource drag; open editors and menus remain interactive, and dragging never triggers the control’s click action. Touch scrolling remains available outside the grips. Picking up a material highlights the entire Desired rates table, including caption and column headers; hovering it strengthens the highlight, and dropping or canceling clears it. Material columns measure actual rendered rate widths (including units), rather than estimating from character counts, and pack beside the small Balance toggle; rates sit beside their icons, with a larger gap after each Balance button. Pool content is enlarged to 130% within its scroll area; the toolbar and app chrome stay unchanged. Circuit gets enough column width for its heading. Machine identity, Tier, and Count have separate labeled columns. Tier retains voltage/amperage and specialized machine controls; Count retains pin editing. Tier, Count, Status, and Power size to their rendered contents. Shared machines span their Tier and Count cells across recipe rows. Narrow worksheets put Tier and Count on a compact labeled controls line. Avoid oversized fixed reservations. Balance hover text states on/off and explains outside supply and surplus, or parent-group sharing, in a short sentence. Yellow chance-percentage overlays on item icons are removed globally; chance information remains in hover details and solver calculations. Takes/Makes rows show compact item icons without inline rates; use the existing item tooltip for rates. Keep non-consumed markers visible. Material rules use a small scales-icon Balance toggle button: off is Match, highlighted is Ignore; hover explains the on/off state and its supply/sharing effect. The Balance control and its explanatory on/off legend share the same scales icon; icon buttons retain accessible Balance labels and pressed state. The explanation also states that materials only made can leave and materials only used can enter. Muted text beside scope headings explains that Balance matches materials made and used here; off allows outside supply/surplus at All production, or sharing with a parent inside a group. This changes presentation only, not saved rules or solver defaults. Preserve saved legacy direct-import rules until the user changes them. Target warnings open their explanation only on request. There is no general never-import rule. Do not restore separate Resources/Materials panels or three-way rule selects.
...
- Pool groups are material scopes, separate from physical shared machines and canvas boards.
  ...
  one-sided resources; share bubbles both sides; import adds a private unlimited source.
