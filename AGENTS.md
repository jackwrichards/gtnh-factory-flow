...
- The UI follows the reference: global Desired rates, All production grand total, nested group
  ...
  group Ignore passes both sides to the parent. A compact status strip sits between bounded Desired rates and right-aligned power. Its shaded Whole setup panel covers every group. It shows live success/failure, actual recipe activity and boundary counts; the All production material-rule counts are root-only. Material rules opens colored examples and per-scope counts (outside supply separate from parent sharing) without expanding the layout. Target warnings focus their explanation. There is no general never-import rule. Do not restore separate Resources/Materials panels or three-way rule selects.
