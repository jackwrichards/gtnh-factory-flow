...
it("keeps root material-rule counts separate from child group rules", () => {
  ...
  fireEvent.click(within(status).getByText("Material rules", { selector: "summary" }));
  expect(within(status).getByRole("region", { name: "Material balance example" })).toBeTruthy();
  expect(within(status).getByRole("group", { name: "Match example" })).toBeTruthy();
  expect(within(status).getByText("70 missing")).toBeTruthy();
  expect(within(status).getByText("70 imported")).toBeTruthy();
