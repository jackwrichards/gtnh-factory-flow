...
describe("describeEdgeRate", () => {
  ...
  it("keeps lines into storage dead simple: no status, no percent, one sentence", () => {
    // Even when the solver flags the edge, a barrel takes whatever arrives.
    const edge = makeEdge({
      demand: 3,
      transferred: 3,
      nameplateDemand: 111,
    ...
    expect(isEdgeStarved(edge)).toBe(false);
    expect(isEdgeSurplus(edge)).toBe(false);
    expect(formatEdgeRateLabel(edge)).toBe("3/s");
    expect(describeEdgeRate(edge)).toBe("Flows into storage at 3/s.");
  });
